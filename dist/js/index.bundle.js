...
function () {
  function RectRoulette(config) {
    ...
    this.view = new RectRouletteView_1.RectRouletteView(config); //Config偵聽事件
  ...
  /**
   * 轉動輪盤
   * @param {('uid' | 'name')} type 指定類型
   * @param {string} val 值
   * @param {(targetRect?:RectItem, rects?:RectItem[]) => void} [onStart] 轉動開始處裡器
   * @param {(nowRect?:RectItem, rects?:RectItem[]) => void} [onChange] 變更時處理器
   * @param {(targetRect?:RectItem, rects?:RectItem[]) => void} [onComplete] 轉動完成處裡器
   * @returns
  ...
  RectRoulette.prototype.SpinTo = function (type, val, _onStart, onChange, _onComplete) {
    ...
    if (this.config.perRectTime < 0.02) //小餘0.02會出現異常
      this.config.perRectTime = 0.02;
    var rects = this.GetRectItem('all'); //項目陣列

    var amount = target.index - this.nowId + rects.length * this.config.cycles; //變動總數

    var totalTime = amount * this.config.perRectTime; //總時間
  ...
  RectRoulette.prototype.GetRectItem = function (type, val) {
    switch (type) {
      case 'uid':
        ...
        if (target != null) return target;else throw new Error('[' + this.constructor.name + ' GetRectItem] 矩形物件不存在 uid: ' + val);

      case 'name':
        var rectItems = this.view.rectContainer.children.toArray().filter(function (rectItem) {
          return rectItem.name() == val;
        });
        if (rectItems.length != 0) return rectItems;else throw new Error('[' + this.constructor.name + ' GetRectItem] 矩形物件不存在 name: ' + val);
...
function (_super) {
  ...
  Object.defineProperty(RectRouletteConfig.prototype, "perRectTime", {
    ...
    /** 每個項目的時間基數，可設定到小數點2位，越小越快 (預設:0.05) */
    set: function set(val) {
      val = parseFloat(val.toFixed(2)); //取小數點2位
...
function (_super) {
   ...
   * 設定底圖
   * @private
   * @param {(string | HTMLImageElement)} src 底圖資源
   * @memberof RectItem
   */
  ...
  /**
   * 設定外框
   * @param {(string | HTMLImageElement)} src 外框資源
   * @memberof RectItem
   */


  RectItem.prototype.SetFrame = function (src) {
    this.VO.frameSrc = src; //清除外框設定
  ...
  RectItem.prototype.SetFrameWidth = function (val) {
    ...
    this.VO.frameWidth = val; //重置外框大小、寬高、筆畫寬度
  ...
  /**
   * 設定獎勵物件
   * @param {(string | HTMLImageElement | Konva.Shape)} src 獎勵物件資源
   * @memberof RectItem
   */
  ...
  RectItem.prototype.SetReward = function (src) {
    ...
    if (this.reward != null) this.reward.destroy(); //清除獎勵物件

    if (src != ( false || undefined)) {
      if (src instanceof HTMLImageElement) //圖片
        {
        ...
        } else if (typeof src == 'string') //文字
        {
          ...
          var txt = this.reward;
          txt.fontSize(txt.fontSize() * this.width() / txt.width() * 0.7); //字體大小70%
        } else if (src instanceof konva_1["default"].Shape) //konva物件
        {
          this._reward = src;
        } //置中
...
function (_super) {
   ...
   * @param {IRectItem[]} iRectItems 項目設定
   * @memberof RectRouletteView
   */
   ...
   * @private
   * @param {string} txt 文字
   * @param {string} [fillColor='black'] 底色
   * @returns {Konva.Label}
   * @memberof RectRouletteView
   */
...
function Init() {
  ...
  ResetSpecificList('uid'); //初始設定選擇清單
  //切換uid事件
...
function () {
   ...
   * @param {{width:number, height:number}} [resolution] 解析度 (預設1024*768)
   * @memberof KonvaBase
   */
  ...
  KonvaBase.prototype.Init_container = function (selector) {
    this.container = document.querySelector(selector); //檢查容器

    if (this.container == null || this.container == undefined) throw new Error('[' + this.constructor.name + ' Init_container] 無法取得容器: ' + selector);else if (this.container.tagName != 'DIV') throw new Error('[' + this.constructor.name + ' Init_container] 容器必須為DIV');
  };
  /**
   * 初始化-場景
   * @private
...
function () {
   ...
   * 開始高亮閃爍
   * @static
   * @param {Konva.Node} konvaNode konva物件
   * @param {number} [count=-1] 閃爍次數 (預設-1<無限>)
   * @param {number} [duration=0.3] 閃爍時間 (預設0.3秒，設定0可保持高亮)
   * @param {number} [brightness=0.5] 亮度 (預設0.5)
   * @returns {TweenMax}
   * @memberof Flicker
   */
  ...
  Flicker.Start = function (konvaNode, count, duration, brightness) {
    ...
    if (count >= 0) count = count * 2 - 1; //配合yoyo計算重複次數
...
function () {
  function EventDispatcher() {
    this.lisenters = EventDispatcher.lisenters; //參照靜態Map

    this.scope = this.constructor.name; //設定偵聽範圍
  }
  /**
   * 加入偵聽
   ...
   * 發送事件
   * @param {string} type 事件名稱
   * @param {boolean} [bubble=false] 氣泡事件
   * @param {...any} arg 自帶參數
   * @memberof EventDispatcher
   */
      ...
      return __generator(this, function (_a) {
        ...
        srcList.forEach(function (src, index) {
          var promise = new Promise(function (resolve, reject) {
            ...
            throw new Error('[' + _this.name + '] 無法載入圖片：' + src);
          });
